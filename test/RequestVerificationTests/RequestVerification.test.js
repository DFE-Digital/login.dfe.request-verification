...
describe("When verifying the request", () => {
  ...
  it("the interactions certification is loaded", () => {
    ...
    fs.readFileSync.mockReturnValue(expectedPublicKey);

    requestVerification.verifyRequest(
      `{"uuid":"${expectedUuid}","uid":"${expectedUid}"}`,
      expectedCert,
      expectedSig,
    );
  ...
  it("the verify function is called passing in the signature and public key and contents", () => {
    ...
    expect(DigitalSignatureService.verifyRequest.mock.calls[0][0]).toBe(
      expectedContents,
    );
    expect(DigitalSignatureService.verifyRequest.mock.calls[0][1]).toBe(
      expectedPublicKey,
    );
    expect(DigitalSignatureService.verifyRequest.mock.calls[0][2]).toBe(
      expectedSig,
    );
  });
